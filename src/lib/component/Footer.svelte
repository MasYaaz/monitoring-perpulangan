...
<footer class="relative w-full font-primary overflow-hidden pt-20">
	...
	<!-- Overlay Transparan -->
	<div
		class="absolute inset-0 bg-gradient-to-t from-secondary/60 via-secondary/80 to-secondary/100 dark:from-primary/90 dark:via-primary/95 dark:to-primary/100"
	></div>
		...
		<div class="px-6 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
			...
			<div>
				...
				<p class="text-sm text-justify max-w-md">
					Terima kasih telah mengunjungi portofolio saya. Jangan ragu untuk menghubungi saya untuk
					kerja sama atau proyek!
				</p>
			</div>
					...
					{#each navItems as nav}
						<li>
							<button
								...
								class="hover:scale-105 hover:cursor-pointer">{nav.icon}</button
							>
						</li>
					{/each}
				</ul>
